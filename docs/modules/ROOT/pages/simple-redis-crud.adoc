...
[source,java,linenums,highlight='16,19..20']
----
@Component
...
. **Line 17**: Serializes the object
. **Line 20**: Requests a connection resource and releases it after the end of the block try
. **Line 21**: Stores the byte array with parameter key (String)
